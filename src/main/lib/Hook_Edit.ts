...
export const editTipo_Mantenimiento_By_Id = async (id, updatedTipoMantenimientoData) => {
  try {
    const [updatedCount, [updatedTipoMantenimiento]] = await Tipo_Mantenimiento.update(updatedTipoMantenimientoData, {
...
export const editEstados_Revision_By_Id = async (id, updatedEstadoRevisionData) => {
  try {
    const [updatedCount, [updatedEstadoRevision]] = await Estados_Revision.update(updatedEstadoRevisionData, {
      where: { ID_Estado: id },
      returning: true,
    });
    if (updatedCount === 0) {
...
export const editOrden_Mantenimiento_By_Id = async (id, updatedOrdenMantenimientoData) => {
  try {
    const [updatedCount, [updatedOrdenMantenimiento]] = await Orden_Mantenimiento.update(updatedOrdenMantenimientoData, {
      where: { ID_Orden: id },
      returning: true,
    });
    if (updatedCount === 0) {
